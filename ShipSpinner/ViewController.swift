...
class ViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, SCNSceneRendererDelegate {
  ...
  func loadShipData(id : NSString) {
    ...
    var webViewText : (String, NSURL) = frame.presenter!.getShipWebViewText(id)
    var text = webViewText.0
    var base = webViewText.1
    detailWebView.loadHTMLString(text, baseURL:base)
